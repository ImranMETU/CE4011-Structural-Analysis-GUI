...
def plot_story_drift_profile(drift_result: dict[str, Any], ax=None):
    ...
    units = drift_result.get("units")
    ax.set_xlabel(f"Story drift ({drift_result.get('direction', 'ux')}) [{unit_label('displacement', units)}]")
    ax.set_ylabel(f"Elevation [{unit_label('length', units)}]")
...
def plot_drift_ratio_profile(drift_result: dict[str, Any], ax=None):
    ...
    ax.set_ylabel(f"Elevation [{unit_label('length', drift_result.get('units'))}]")
...
def plot_floor_displacement_profile(floor_displacement_result: dict[str, Any], ax=None):
    ...
    units = floor_displacement_result.get("units")
    ax.set_xlabel(f"Floor displacement ({floor_displacement_result.get('direction', 'ux')}) [{unit_label('displacement', units)}]")
    ax.set_ylabel(f"Elevation [{unit_label('length', units)}]")
...
def plot_roof_displacement_marker(roof_result: dict[str, Any], ax=None):
    ...
    units = roof_result.get("units")
    ax.set_xlabel(f"Roof displacement ({roof_result.get('direction', 'ux')}) [{unit_label('displacement', units)}]")
    ax.set_ylabel(f"Elevation [{unit_label('length', units)}]")
